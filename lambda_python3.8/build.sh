...
if [[ $CR_USER ]] && [[ $CR_PAT ]]
then
  docker tag bepro/lambda_python3.8:latest ghcr.io/bepro-company/docker/lambda_python3.8:latest
  echo $CR_PAT | docker login ghcr.io -u $CR_USER --password-stdin
  docker push ghcr.io/bepro-company/docker/lambda_python3.8:latest
else
